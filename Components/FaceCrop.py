        ...
        if count < len(Frames):
            if len(faces) > -1:
                ...
                try:
                    ...
                    print(e)
                    if isinstance(Frames[count], (list, tuple)) and len(Frames[count]) > 0:
                ...
                if count == 0 or (x_start - (centerX - half_width)) < 1:
                    ...
                    if int(frame.shape[1]) != x_end - x_start:
                        if x_end < original_width:
                            x_end += int(frame.shape[1]) - (x_end - x_start)
                            if x_end > original_width:
                                x_start -= int(frame.shape[1]) - (x_end - x_start)
                        else:
                            x_start -= int(frame.shape[1]) - (x_end - x_start)
                            if x_start < 0:
                                x_end += int(frame.shape[1]) - (x_end - x_start)
